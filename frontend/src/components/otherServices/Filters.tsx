...
export const Filters = ({
  ...
  return (
    ...
    >
      <button
        className="absolute right-2 top-2 inline-flex items-center text-xs font-bold text-white sm:text-sm"
...
export const FiltersSummary = ({
  ...
  return (
    ...
    >
      <button
        id="openFilters"
        className="absolute right-2 top-2 flex items-center justify-center rounded bg-slate-50 p-2 text-xs font-semibold text-gray-600 shadow sm:text-sm"
        onClick={() => setIsFilterOpen(!isFilterOpen)}
        type="button"
      >
      ...
      <p className="mb-5 basis-full text-2xl underline decoration-sky-500 decoration-wavy decoration-2 underline-offset-8">
        فلتــرة الخدمات الاخرى
      </p>
      <span>{" الخدمات الاخرى المحفوظة"}</span>
      {!searchQuery && (
