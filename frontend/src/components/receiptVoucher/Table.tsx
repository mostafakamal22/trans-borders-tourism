...
import { receiptVoucherTableHeaderTitles } from "./constants";
...
export const tableHeader = (
  <tr className="border-y border-y-black">
    {receiptVoucherTableHeaderTitles.map((title) => (
      <th
        key={title}
        scope="col"
...
export const tableRow = ({
  basicOptions,
  extraOptions,
}: TableRowProps<IReceiptVoucherDocument>) => {
  const { item: receiptVoucher, index } = basicOptions;
  const { handleRemoving, isDeleting, setId, setIsOpen } = extraOptions;

  return (
    <tr
      key={receiptVoucher?.id}
      className={`${
    ...
    >
      ...
      >
        <form
          ...
          onSubmit={(event) => handleRemoving(event, receiptVoucher?.id)}
        >
          <FormButton
      ...
      >
        <Link
          to={`/receiptVouchers/${receiptVoucher?.id}`}
      ...
      <th
        ...
        className="border-x border-x-black p-1 text-center text-gray-900"
      >
        {dayjs(receiptVoucher?.payment_date).format("DD/MM/YYYY")}
      </th>
