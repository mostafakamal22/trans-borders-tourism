...
import { useGetOnePaymentVoucherQuery } from "../../state/features/paymentVoucher/paymentVouchersApiSlice";
...
export default function PaymentVoucher() {
  ...
  const paymentVoucherID = useParams()?.id || "";
  ...
  const {
    ...
    isLoading,
    error,
  } = useGetOnePaymentVoucherQuery({ id: paymentVoucherID });
  ...
  useEffect(() => {
    if (paymentVoucher) {
      ...
      setPaymentVoucherDetails({
        name: paymentVoucher.customer_name,
        bank: paymentVoucher?.bank || "",
        referenceNumber: paymentVoucher?.reference_number || "",
        being: paymentVoucher?.being || "",
        amount: paymentVoucher.amount,
        paymentDate: paymentVoucher?.payment_date
          ? dayjs(paymentVoucher?.payment_date).format("YYYY-MM-DD")
          : "",
      });
    }
  }, [paymentVoucher]);
  ...
  if (!paymentVoucher || isLoading) return <DataFetchingSpinner />;
